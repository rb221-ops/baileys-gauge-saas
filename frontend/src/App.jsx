...
const App = () => {
  ...
  const generateChartData = () => {
    return Array.from({ length: 100 }, (_, i) => ({
      time: `${Math.floor(i / 4)}:${String((i % 4) * 15).padStart(2, '0')}`,
  ...
  const addAlert = () => {
    ...
    setAlerts((prev) => [
      ...prev,
      { id: Date.now(), ticker: alertTicker.toUpperCase(), price: alertPrice },
    ]);
    setAlertTicker('');
    setAlertPrice('');
  };
  ...
  const handleChartMouseUp = (e) => {
    ...
    const newLine = {
      ...
      color: drawMode === 'trendline' ? '#00d9ff' : '#ff6b6b',
      type: drawMode,
    };
  ...
  const handleUpgrade = (tier) => {
    alert(
      `Redirecting to payment for ${tier}... (Stripe integration)\n\nThis would charge:\n${
    ...
    return (
      <div style={{ background: 'linear-gradient(135deg, #0a0a14 0%, #1a1a2e 100%)', minHeight: '100vh', color: 'white', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto', padding: mobileView ? '2rem 1rem' : '5rem 2rem' }}>
          <h2 style={{ fontSize: mobileView ? '36px' : '64px', fontWeight: '900', margin: '0 0 1rem', color: '#00d9ff', lineHeight: '1.1' }}>
            Professional Day Trading<br />Command Center
          </h2>
          <p style={{ fontSize: mobileView ? '14px' : '18px', color: '#aaa', margin: '0 0 2rem', maxWidth: '700px' }}>
            Advanced technical analysis. Real-time alerts. Draw support/resistance. Find winning trades before they move. Professional-grade tools trusted by 1000+ day traders.
          </p>

          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '4rem' }}>
            <button onClick={() => { setIsSignUp(true); setPage('auth'); }} style={{ padding: '1rem 2.5rem', background: '#00d9ff', color: '#0a0a14', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '700', fontSize: '16px', boxShadow: '0 20px 40px rgba(0,217,255,0.3)' }}>
              Start 7-Day Free Trial →
            </button>
            <button onClick={() => setShowDemoModal(true)} style={{ padding: '1rem 2.5rem', background: 'transparent', color: '#00d9ff', border: '2px solid #00d9ff', borderRadius: '8px', cursor: 'pointer', fontWeight: '700', fontSize: '16px', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <div style={{ background: 'rgba(0,217,255,0.05)', border: '1px solid rgba(0,217,255,0.2)', padding: mobileView ? '2rem 1rem' : '3rem 2rem', borderRadius: '12px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: mobileView ? '1fr' : 'repeat(2, 1fr)', gap: '2rem' }}>
              {[
                {
                  tier: 'Trial', price: 'FREE', days: '7 Days',
                  features: ['Full access to all scanners', '50+ technical indicators', 'Draw support/resistance', 'Price alerts (10)', 'Watchlist (50 stocks)', 'No credit card required'],
                },
                {
                  tier: 'Pro', price: '$29', period: '/month',
                  features: ['Unlimited scans', 'All 50+ indicators', 'Unlimited alerts', 'Draw tools', 'Unlimited watchlists', '1-on-1 support'],
                  highlighted: true,
                },
              ].map((plan, i) => (
                <div key={i} style={{ background: plan.highlighted ? 'rgba(0,217,255,0.15)' : 'transparent', border: plan.highlighted ? '2px solid #00d9ff' : '1px solid rgba(0,217,255,0.2)', padding: '2rem', borderRadius: '12px' }}>
                  ...
                  <button onClick={() => { setIsSignUp(true); setPage('auth'); }} style={{ width: '100%', padding: '0.75rem', background: plan.highlighted ? '#00d9ff' : 'transparent', color: plan.highlighted ? '#0a0a14' : '#00d9ff', border: plan.highlighted ? 'none' : '1px solid #00d9ff', borderRadius: '8px', cursor: 'pointer', fontWeight: '700', fontSize: '14px' }}>
                    Get Started
                  </button>
                </div>
              ))}
    ...
    return (
      <div style={{ background: 'linear-gradient(135deg, #0a0a14 0%, #1a1a2e 100%)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
        <div style={{ maxWidth: '450px', width: '100%', background: 'rgba(26,26,46,0.8)', padding: '3rem', borderRadius: '12px', border: '1px solid rgba(0,217,255,0.1)' }}>
          <h2 style={{ textAlign: 'center', color: 'white', marginTop: 0, marginBottom: '1rem', fontSize: '28px', fontWeight: '700' }}>
            {isSignUp ? 'Create Account' : 'Sign In'}
          </h2>
          <p style={{ textAlign: 'center', color: '#aaa', margin: '0 0 2rem', fontSize: '13px' }}>
          ...
          <p style={{ textAlign: 'center', color: '#aaa', marginTop: '1.5rem', fontSize: '13px' }}>
            ...
            <button onClick={() => setIsSignUp(!isSignUp)} style={{ background: 'none', border: 'none', color: '#00d9ff', cursor: 'pointer', fontWeight: '700' }}>
              {isSignUp ? 'Sign In' : 'Sign Up'}
            </button>
          </p>
          <button onClick={() => setPage('landing')} style={{ width: '100%', marginTop: '1rem', padding: '0.75rem', background: 'transparent', color: '#aaa', border: '1px solid rgba(0,217,255,0.2)', borderRadius: '8px', cursor: 'pointer' }}>
            ← Back to Home
          </button>
        </div>
      </div>
      ...
      <div style={{ background: 'linear-gradient(135deg, #0a0a14 0%, #1a1a2e 100%)', minHeight: '100vh', color: 'white', display: 'flex' }}>
        ...
        {sidebarOpen && (
          <div style={{ width: mobileView ? '100%' : '280px', background: 'rgba(26,26,46,0.8)', borderRight: '1px solid rgba(0,217,255,0.1)', padding: '1.5rem', overflowY: 'auto', maxHeight: '100vh', position: mobileView ? 'fixed' : 'relative', zIndex: 100 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
              ...
              {mobileView && (
                <button onClick={() => setSidebarOpen(false)} style={{ background: 'none', border: 'none', color: '#00d9ff', cursor: 'pointer', fontSize: '20px' }}>✕</button>
              )}
            </div>
            ...
            {categories.map((cat) => (
              <button key={cat.id} onClick={() => setSelectedCategory(cat.id)} style={{ width: '100%', padding: '0.75rem', background: selectedCategory === cat.id ? 'rgba(0,217,255,0.15)' : 'transparent', color: selectedCategory === cat.id ? '#00d9ff' : '#aaa', border: 'none', textAlign: 'left', cursor: 'pointer', fontSize: '13px', fontWeight: selectedCategory === cat.id ? '600' : '400', borderLeft: selectedCategory === cat.id ? '3px solid #00d9ff' : '3px solid transparent', transition: 'all 0.2s', marginBottom: '0.5rem' }}>
                {cat.name}
              </button>
            ))}
            ...
            <div style={{ borderTop: '1px solid rgba(0,217,255,0.1)', paddingTop: '1.5rem', marginTop: '1.5rem' }}>
              ...
              {[
                ...
                { id: 'watchlist', label: `❤️ Watchlist (${watchlist.length})` },
                { id: 'alerts', label: `🔔 Alerts (${alerts.length})` },
              ].map((nav) => (
                <button key={nav.id} onClick={() => setActiveTab(nav.id)} style={{ width: '100%', padding: '0.75rem', background: activeTab === nav.id ? 'rgba(0,217,255,0.15)' : 'transparent', color: activeTab === nav.id ? '#00d9ff' : '#aaa', border: 'none', textAlign: 'left', cursor: 'pointer', fontSize: '13px', fontWeight: activeTab === nav.id ? '600' : '400', borderLeft: activeTab === nav.id ? '3px solid #00d9ff' : '3px solid transparent', marginBottom: '0.5rem' }}>
                  {nav.label}
                </button>
              ))}
            </div>
            ...
            <div style={{ position: 'absolute', bottom: '1.5rem', left: '1.5rem', right: '1.5rem' }}>
              {subscriptionTier === 'trial' && (
                <button onClick={() => handleUpgrade('pro')} style={{ width: '100%', padding: '0.75rem', background: '#00d9ff', color: '#0a0a14', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '700', fontSize: '12px', marginBottom: '0.75rem' }}>
                  Upgrade to Pro
                </button>
              )}
              <button onClick={handleLogout} style={{ width: '100%', padding: '0.75rem', background: 'transparent', color: '#ff6b6b', border: '1px solid rgba(255,107,107,0.3)', borderRadius: '6px', cursor: 'pointer', fontWeight: '600', fontSize: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
        ...
        <div style={{ flex: 1, padding: mobileView ? '1rem' : '2rem', overflowY: 'auto', maxHeight: '100vh' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', gap: '1rem', flexWrap: 'wrap' }}>
            {!sidebarOpen && (
              <button onClick={() => setSidebarOpen(true)} style={{ background: 'none', border: 'none', color: '#00d9ff', cursor: 'pointer', fontSize: '24px' }}>☰</button>
            )}
            {activeTab === 'screener' && (
              <form onSubmit={handleScan} style={{ display: 'flex', gap: '0.5rem', flex: 1, minWidth: mobileView ? '100%' : '300px' }}>
                ...
                <button type="submit" disabled={loading} style={{ padding: '0.75rem 1.5rem', background: '#00d9ff', color: '#0a0a14', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: '700', fontSize: '12px', opacity: loading ? 0.5 : 1 }}>
          ...
          {activeTab === 'screener' && (
            <>
              {scanResults.length > 0 && (
                <>
                  <h2 style={{ margin: '1rem 0 2rem', fontSize: mobileView ? '18px' : '24px', fontWeight: '700' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: mobileView ? '1fr' : 'repeat(auto-fit, minmax(550px, 1fr))', gap: '2rem', marginBottom: '2rem' }}>
                    {scanResults.map((stock, i) => (
                      <div key={i} onClick={() => { setSelectedStock(stock); setChartData(generateChartData()); setLines([]); }} style={{ background: 'rgba(0,217,255,0.05)', border: selectedStock?.ticker === stock.ticker ? '2px solid #00d9ff' : '1px solid rgba(0,217,255,0.2)', borderRadius: '12px', padding: mobileView ? '1rem' : '1.5rem', cursor: 'pointer', transition: 'all 0.3s' }}>
                        ...
                        <p style={{ margin: '0 0 1rem', color: Number(stock.change) > 0 ? '#4ade80' : '#ff6b6b', fontWeight: '700', fontSize: '16px' }}>
                          {Number(stock.change) > 0 ? '▲' : '▼'} {stock.change}% ({stock.percentChange}%)
                        </p>
                ...
                <div style={{ background: 'rgba(0,217,255,0.05)', border: '1px solid rgba(0,217,255,0.2)', borderRadius: '12px', padding: mobileView ? '1rem' : '2rem', marginBottom: '2rem' }}>
                  ...
                  <div style={{ position: 'relative', background: 'rgba(10,10,20,0.5)', borderRadius: '8px', overflow: 'hidden', marginBottom: '1.5rem', border: '1px solid rgba(0,217,255,0.1)', height: mobileView ? '300px' : '500px' }}>
                    ...
                    <canvas
                      ...
                      style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', cursor: drawMode ? 'crosshair' : 'default' }}
                    />
                  </div>
              ...
              {scanResults.length === 0 && (
                <div style={{ textAlign: 'center', padding: '4rem 2rem', color: '#666' }}>
                  <TrendingUp size={48} style={{ margin: '0 auto 1rem', display: 'block', opacity: 0.3 }} />
                  <p style={{ fontSize: mobileView ? '14px' : '16px', margin: 0 }}>
                    Enter stock tickers to scan for {categories.find((c) => c.id === selectedCategory)?.name}
                  </p>
          ...
          {activeTab === 'watchlist' && (
            <>
              {watchlist.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '4rem 2rem', color: '#666' }}>
                  <Heart size={48} style={{ margin: '0 auto 1rem', display: 'block', opacity: 0.3 }} />
                ...
                <div style={{ display: 'grid', gridTemplateColumns: mobileView ? '1fr' : 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1.5rem' }}>
                  {scanResults.filter((s) => watchlist.includes(s.ticker)).map((stock, i) => (
                    <div key={i} style={{ background: 'rgba(0,217,255,0.05)', border: '1px solid rgba(0,217,255,0.2)', borderRadius: '12px', padding: '1.5rem' }}>
                      ...
                      <p style={{ margin: '0.5rem 0', color: Number(stock.change) > 0 ? '#4ade80' : '#ff6b6b', fontWeight: 'bold', fontSize: '14px' }}>
                      ...
                      <button onClick={() => { setAlertTicker(stock.ticker); setActiveTab('alerts'); }} style={{ width: '100%', padding: '0.75rem', background: 'rgba(0,217,255,0.2)', color: '#00d9ff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: '600', fontSize: '12px' }}>
                        Set Alert
                      </button>
                    </div>
                  ))}
          ...
          {activeTab === 'alerts' && (
            <>
              ...
              {alerts.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '4rem 2rem', color: '#666' }}>
                  <AlertCircle size={48} style={{ margin: '0 auto 1rem', display: 'block', opacity: 0.3 }} />
                ...
                <div style={{ display: 'grid', gridTemplateColumns: mobileView ? '1fr' : 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1rem' }}>
                  {alerts.map((alert, i) => (
                    <div key={i} style={{ background: 'rgba(0,217,255,0.05)', border: '1px solid rgba(0,217,255,0.2)', borderRadius: '12px', padding: '1.5rem' }}>
                      ...
                      <button onClick={() => setAlerts((prev) => prev.filter((a) => a.id !== alert.id))} style={{ width: '100%', marginTop: '1rem', padding: '0.5rem', background: '#ff6b6b', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: '600', fontSize: '11px' }}>
                        Delete
                      </button>
                    </div>
                  ))}
